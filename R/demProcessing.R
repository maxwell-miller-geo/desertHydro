...
flow_accumlation_wb <- function(dem_file_path, Outpath, watershed_shape_path = NA, smooth_tif = "smoothed_dem.tif", filled_dem = "filled_dem.tif", breached_tif = "breached.tif", out_accum = "flow_accumulation.tif", max_change = 1){
  ...
  if(requireNamespace("whitebox") & requireNamespace("terra")){
    # Initialize whitebox
    #whitebox::wbt_init()
    if(!is.na(watershed_shape_path)){
      ...
      if(dem_code == shape_crs_code){
        ...
        writeRaster(clipped_dem, filename = cropped_dem, overwrite = T) # creates cropped
  ...
  whitebox::wbt_breach_depressions_least_cost(dem = cropped_dem, output = model_dem, dist = 1, max_cost = max_change)
