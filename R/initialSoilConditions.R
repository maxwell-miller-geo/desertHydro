...
storage_amount <- function(landCoverTable){
  ...
  landCoverTable$maximumStorageAmount <- (1- landCoverTable$rockPercent) * landCoverTable$saturatedMoistureContent * landCoverTable$soilDepthCM
  # Create a map??
...
initial_soil_conditions <- function(LandCoverCharacteristics, ClassificationMap, DEM, ModelOutputs, type = "LC", saturatedPercentage = 0.2){
  ...
  LCC$saturatedHydraulicMatrix <- LCC$saturatedHydraulicMatrix / hour_to_min
  LCC$verticalHydraulicConductivity <- LCC$verticalHydraulicConductivity / hour_to_min
  LCC$hydraulicConductivityRestrictiveLayer <- LCC$hydraulicConductivityRestrictiveLayer / hour_to_min
